...
class ChatController extends Controller
{
        ...
        try {
            ...
            $response = $client->post($url, [
                'json' => [
                    ...
                    'messages' => [
                        ['role' => 'system', 'content' => 'Bạn là MAMNON AI 🌸, trợ lý ảo chuyên nghiệp và thân thiện của trường mầm non. Nhiệm vụ: Hỗ trợ phụ huynh, giải đáp thắc mắc về thực đơn, học phí, hoạt động. QUAN TRỌNG: TUYỆT ĐỐI DÙNG 100% TIẾNG VIỆT để trả lời. Không sử dụng tiếng Anh ngay cả khi thiếu dữ liệu. Cấu trúc: (1) Chào thân thiện, (2) Thông tin chính (gạch đầu dòng), (3) Lời kết/gợi ý. Sắc thái: Nhiệt tình, chu đáo, yêu trẻ.'],
    ...
    /**
     * Helper: Làm sạch chuỗi UTF-8 để tránh lỗi JSON Encode
     */
    ...
    {
        // Ưu tiên lấy thực đơn từ hôm nay trở đi
        $thucDons = ThucDon::where('ngay', '>=', date('Y-m-d'))
            ->orderBy('ngay', 'asc')
            ->take(7)
            ->get();

        // Nếu không có, lấy thực đơn gần nhất trong quá khứ
        if ($thucDons->isEmpty()) {
            $thucDons = ThucDon::orderBy('ngay', 'desc')->take(5)->get()->reverse();
        }
        ...
        return "📅 THỰC ĐƠN:\n" . $thucDons->map(function ($td) {
            ...
            $ten = $this->cleanString($td->ten_thuc_don);
            $moTa = $this->cleanString($td->mo_ta);
            return "- {$ten} (Ngày: {$ngay}): {$moTa}";
        })->implode("\n");
    }
    ...
    {
        // Lấy danh sách ID của các thực đơn mới nhất (để lấy chi tiết tương ứng)
        $recentThucDonIds = ThucDon::orderBy('ngay', 'desc')->take(7)->pluck('id');

        $chiTiet = ChiTietThucDon::with(['thucDon', 'monAn', 'buaAn'])
            ->whereIn('id_thuc_don', $recentThucDonIds)
            ->get();
        ...
        return "🍱 CHI TIẾT MÓN ĂN:\n" . $chiTiet->map(function ($ct) {
            $ngay = isset($ct->thucDon->ngay) ? date('d/m/Y', strtotime($ct->thucDon->ngay)) : 'N/A';
            $tenBua = $this->cleanString($ct->buaAn->ten_bua_an ?? 'Bữa ăn');
            $tenMon = $this->cleanString($ct->monAn->ten_mon ?? 'Món ăn');
            return "- [{$ngay}] {$tenBua}: {$tenMon}";
        })->implode("\n");
    }
    ...
    private function getBuaAnData()
    {
        ...
        return "🍽️ DANH SÁCH BỮA ĂN:\n" . $buaAn->map(function ($b) {
            $ten = $this->cleanString($b->ten_bua_an);
            $moTa = $this->cleanString($b->mo_ta);
            return "- {$ten}: {$moTa}";
        })->implode("\n");
    }
    ...
    private function getMonAnData()
    {
        ...
        return "🍲 DANH SÁCH MÓN ĂN (Tham khảo):\n" . $monAn->map(function ($m) {
            $ten = $this->cleanString($m->ten_mon);
            return "- {$ten} (Calo: {$m->calo}, Đạm: {$m->protein})";
        })->implode("\n");
    }
    ...
    private function getBaiVietData()
    {
        ...
        return "📰 TIN TỨC & BÀI VIẾT MỚI:\n" . $baiViets->map(function ($bv) {
            $tieuDe = $this->cleanString($bv->tieu_de);
            $noiDung = $this->cleanString(\Illuminate\Support\Str::limit(strip_tags($bv->noi_dung), 100));
            return "- [{$bv->ngay_dang}] {$tieuDe}: {$noiDung}";
        })->implode("\n");
    }
...
3. **Context dữ liệu:**
   - Dựa vào dữ liệu cung cấp bên dưới để trả lời.
   ...
   - **Xử lý khi thiếu dữ liệu:**
     • Các thông tin cá nhân/học phí: Trả lời "Hiện chưa có thông tin trên hệ thống".
     • **Riêng về THỰC ĐƠN/MÓN ĂN**: Nếu dữ liệu hệ thống không có hoặc bị trống, BẠN HÃY TỰ ĐỀ XUẤT một thực đơn dinh dưỡng, khoa học và hấp dẫn cho trẻ mầm non (Sáng/Trưa/Chiều) phù hợp với ngày được hỏi.
